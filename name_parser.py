...
class FontnameParser:
    ...
    def camel_explode(word):
        ...
        (prefix, word) = m.group(1, 2) if m is not None else ("", word)
        if len(word) == 0:
            return prefix
    ...
    def parse_font_name(name):
        ...
        familyname = FontnameParser.camel_casify(matches.group(1))  # type: ignore
        style = matches.group(2)  # type: ignore
        ...
        if "-" in style:
            ...
            return FontnameParser.parse_font_name(
                familyname + matches.group(1) + "-" + matches.group(2)  # type: ignore
            )
    ...
    def fullname(self):
        ...
        if ("Regular" in styles) and (
            not self.keep_regular_in_family or len(self.weight_token) > 0
    ...
    def subfamily(self):
        ...
        if len(self.style_token) == 0:
            ...
            return "Regular"
        if "Oblique" in self.weight_token and "Italic" not in self.style_token:
            return FontnameParser.concat(self.style_token, "Italic")
